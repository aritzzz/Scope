...
ACC.rename(columns = {'b':'kwmatch', 'c':'clustering', 'd':'bibsum', 'e':'bibtitle', 'f':'bibjournal', 'g':'bibconf', 'h':'adpf', 'bibtitle':'btitle'}, inplace = True)
REJ.rename(columns = {'b':'kwmatch', 'c':'clustering', 'd':'bibsum', 'e':'bibtitle', 'f':'bibjournal', 'g':'bibconf', 'h':'adpf', 'bibtitle':'btitle'}, inplace = True)
data = list(ACC.columns)[2:-1]
print data
...
for i in range(len(data)):
	...
	ax.plot(bin_edges[1:], cdf, c = 'blue', alpha = a, linewidth = 2.0)
	ax.yaxis.set_ticks_position('left')
	ax.xaxis.set_ticks_position('bottom')
	#ax.set_xlabel(data[0])
	hist,bin_edges = np.histogram(REJ[data[i]],bins = 'auto', normed = True)
	cdf = np.cumsum(hist*np.diff(bin_edges))
	ax.plot(bin_edges[1:], cdf, c = 'maroon', alpha = a, linewidth = 2.0)
	ax.set_ylim(0,1.1)
	#t = t + 1
